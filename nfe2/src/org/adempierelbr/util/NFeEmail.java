 ...
 *
 * BF 3048696 - Alteração para padrão texto e adicionada nova mensagem para cancelamento
 *
...
public abstract class NFeEmail {
	...
	public static boolean sendMail(final MNotaFiscal nf)
	{
		...
		try{
			...
			if (from.getEMailUser() == null && from.getEMailUserPW() == null)
				throw new IllegalArgumentException("Problemas com o Contato de emissão da NFe da Organização");
			...
			if (contacts.size() > 0){
				...
				final String message = getMessage(nf.getDocumentNo(), nf.getlbr_OrgName(), nf.getlbr_CNPJ(), nf.isCancelled(), nf.getlbr_NFeID());
							...
							for (final MUser to : contacts){
								...
								if (mailto == null || mailto.indexOf('@') == -1 || mailto.contains(" "))
									throw new IllegalArgumentException("Problems with Client Email");
										...
										for (int i=0;i<30;i++){
											if (client.sendEMail(from, to, subject, message, nfeXML, false))
													break;
	...
	private static final String getMessage(String documentNo, String companyName, String CNPJ, boolean isCancelled, String NFe_ID){
		if(isCancelled)
		{
			StringBuffer messageCancel = new StringBuffer("");
			messageCancel.append("Prezado Cliente, " +
								 "\n" +
								 "\n" +
								 "Você está recebendo o XML referente ao CANCELAMENTO da Nota Fiscal Eletrônica número [" + documentNo + "] da [" + companyName + "], CNPJ [" + CNPJ + "]." +
								 "\n" +
								 "\n" +
							     "A validade e autenticidade deste documento eletrônico pode ser verificada no site nacional do projeto (http://www.nfe.fazenda.gov.br/portal/), através da " +
							     "Chave de Acesso ["+NFe_ID+"]." +
							     "\n" +
							     "\n" +
								 "--" +
								 "\n" +
								 "ATENCAO: Este e-mail foi gerado automaticamente pelo sistema da "+ companyName + ", por favor não responda.");
			
			return messageCancel.toString();
		}
		else
		{
			StringBuffer messageOk = new StringBuffer("");
			messageOk.append("\n" +
						   "Prezado cliente," +
					   	   "\n" +
					   	   "\n" +
					   	   "Você está recebendo a Nota Fiscal Eletrônica número [" + documentNo + "] da [" + companyName + "], CNPJ [" + CNPJ + "]." +
					   	   "\n" +
					   	   "\n" +
					   	   "Junto com a mercadoria, você receberá também um DANFE (Documento Auxiliar da Nota Fiscal Eletrônica), " +
					       "que acompanha o trânsito das mercadorias, conforme aprovado pelo ajuste SINIEF 07/05 e alterações do ajuste SINIEF 04/06. " +
					       "\n" +
					       "\n" +
					       ...
					       "\n" +
					       "\n" +
					       "Os registros fiscais e contábeis devem ser feitos, a partir do próprio arquivo da NF-e, anexo neste e-mail, " +
					       "ou utilizando o DANFE, que representa graficamente a Nota Fiscal Eletrônica. A validade e autenticidade deste " +
					       "documento eletrônico pode ser verificada no site (http://www.nfe.fazenda.gov.br/portal/), através da " +
					       "Chave de Acesso ["+NFe_ID+"]." +
					       "\n" +
					       "\n" +
					       "O contribuinte destinatário, não emissor de NF-e, poderá utilizar os dados descritos do DANFE para a escrituração da NF-e, " +
					       ...
					       "comprovada através da emissão da Autorização de Uso e consultada pelo destinatário da NF-e." +
					       "\n" +
					       "\n" +
