...
public class WFieldRecordInfo extends Window implements EventListener
{
	 ...
	 *  @param Record_ID
	 */
	public WFieldRecordInfo (String title, String content, int AD_Table_ID, int AD_Column_ID, int Record_ID)
	{
		super ();
		this.setTitle(title);
	 ...
	 */
	private boolean dynInit(String title, String content)
	{
		m_info.append(" (");
	...
	public static void start(GridField gridField) {
		String content = "";
		if (gridField.getValue() instanceof Boolean)
		{
			content = (Boolean) gridField.getValue() ? "Y" : "N";
		}
		else
			content = "" + gridField.getValue();
		//
		new WFieldRecordInfo(gridField.getColumnName(), content, 
