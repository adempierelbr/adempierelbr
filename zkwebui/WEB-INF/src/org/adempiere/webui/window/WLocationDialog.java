...
public class WLocationDialog extends Window implements EventListener
{
	...
	private void initComponents()
	{
		...
		btnGetAddress = new Button();
		btnGetAddress.setImage("/images/Process16.png");
		btnGetAddress.addEventListener(Events.ON_CLICK,this);
	...
	private void init()
	{
		...
		pnlButton.appendChild(btnGetAddress);
	...
	public void onEvent(Event event) throws Exception
	{
		if (btnOk.equals(event.getTarget()))
		{
			...
			if (m_location.getCountry().isHasRegion()) 
			{
				...
				//	Obrigatório selecionar a cidade na lista
				else if (m_location.getCountry().getC_Country_ID() == 139 && txtCity.getC_City_ID() < 1)
				{
					FDialog.error(0, this, "FillMandatory", Msg.parseTranslation (Env.getCtx(), "@C_City_ID@"));
					inOKAction = false;
					return;
				}
			}
		...
		else if (btnGetAddress.equals(event.getTarget()))
		{
					...
					&& !txtPostal.getText().trim().equals(""))
			{
				if (!txtAddress1.getText().equals("")
						|| !txtAddress2.getText().equals("")
						|| !txtAddress3.getText().equals("")
						|| !txtAddress4.getText().equals("")
						|| txtCity.getSelectedIndex() > 0)
				{
					String warningMsg = "O endereço atual será substituido. Deseja continuar?";
					String warningTitle = "Aviso";
					boolean response = FDialog.ask (m_WindowNo, this, warningTitle, warningMsg);
					if (!response)
						return;
				}
