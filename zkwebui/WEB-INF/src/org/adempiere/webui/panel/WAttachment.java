		...
		if (entry != null && entry.getData() != null)
		{
			...
			try
			{
				...
				if ("text/xml".equals(contentType))
				{
					...
					XmlObject xml = XmlObject.Factory.parse(new String(entry.getData()),options);
					bytes = xml.xmlText(options).getBytes();
				}
				AMedia media = new AMedia(entry.getName(), null, contentType, bytes);
