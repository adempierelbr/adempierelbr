...
public class EFDUtil
{
	...
	/**
	 * Retorna o Código da Versão do SPED de acordo com o período do ano
	 * 
	 * @param dateFrom
	 * @return String com código da versão. Ex.: '003' 
	 */
	...
	public static R0000 createR0000(Properties ctx, int AD_Org_ID, Timestamp dateFrom, Timestamp dateTo, String TrxName)
	{
	...
	public static R0150 createR0150(MBPartner bp){
		...
		// cnpj ou cpf
		String COD_PART = "";
		if(bp.get_ValueAsString(I_W_C_BPartner.COLUMNNAME_lbr_BPTypeBR).equals("PF"))
			COD_PART = TextUtil.toNumeric(bp.get_ValueAsString(I_W_C_BPartner.COLUMNNAME_lbr_CNPJ));
		else 
			COD_PART = TextUtil.toNumeric(bp.get_ValueAsString(I_W_C_BPartner.COLUMNNAME_lbr_CPF));
		
		// se for null, então pega o código do PN
		if (COD_PART == null || COD_PART.trim().equals(""))
			COD_PART = TextUtil.toNumeric(bp.getValue());
		...
		String IE       = bp.get_ValueAsString(I_W_C_BPartner.COLUMNNAME_lbr_IE);
		String SUFRAMA  = bp.get_ValueAsString(I_W_C_BPartner.COLUMNNAME_lbr_Suframa);

		/* 
		 * NAO USADO - BUSCADO A INFORMAÇÃO DIRETA DO PN
		 * 
	...
	public static R0190 createR0190(MLBRNotaFiscalLine nfLine){
		...
		MUOM uom = new MUOM(getCtx(),nfLine.getC_UOM_ID(),get_TrxName());
		
		String UNID  = "";
		String DESCR = "";
		if(uom != null)
		{
			UNID = nfLine.getlbr_UOMName() == null? uom.getUOMSymbol() : nfLine.getlbr_UOMName();
			UNID = AdempiereLBR.getUOMDesc_trl(uom);
		}
	...
	*/
	...
	public static RC100 createRC100(MLBRNotaFiscal nf, String COD_PART, String COD_MOD, String IND_EMIT){
		...
		String UF         = FRKUtils.tratarUF(nf.getlbr_BPRegion());
		String IND_OPER   = nf.isSOTrx() ? "1" : "0"; //0 = Entrada, 1 = Saída
		String COD_SIT    = FRKUtils.getCOO_SIT(nf);
		...
		// Parcelado
		MPaymentTerm paymentTerm = new MPaymentTerm(ctx, nf.getC_PaymentTerm_ID(), null);
		if(paymentTerm != null && paymentTerm.getSchedule(false).length > 0)
			IND_PAG    = "1";
		
		BigDecimal VL_DESC 	= nf.getDiscount();
		...
		// BF: Para ativos fixo, lançar o crédito no bloco G - NAO USADO - PABLO
		//BigDecimal[] assetAmt = Env.ZERO; // FIXME nf.getAssetTaxAmt();
		...
		BigDecimal VL_PIS_ST 	= Env.ZERO;
		BigDecimal VL_COFINS_ST = Env.ZERO;
		
		// NF Para Zona de Livre comércio, zera os impostos e coloca o desconto no campo VL_ABAT_NT
		if(FRKUtils.hasSuframa(nf.getlbr_BPSuframa()))
		{
			// valor de desconto dos impostos para a ZonaFranca
			VL_ABAT_NT  = nf.getDiscount();
			
			VL_BC_ICMS 	= Env.ZERO;
			VL_ICMS 	= Env.ZERO;
			VL_BC_ICMS_ST = Env.ZERO;
			VL_ICMS_ST 	= Env.ZERO;
			VL_IPI 		= Env.ZERO;
			VL_PIS 		= Env.ZERO;
			VL_COFINS	= Env.ZERO;
			VL_PIS_ST 	= Env.ZERO;
			VL_COFINS_ST = Env.ZERO;
	...
	public static RC120 createRC120(MLBRNotaFiscalLine nfLine){
		...
		if (!FRKUtils.tratarCFOP(nfLine.getLBR_CFOP_ID()).startsWith("3"))
			return null;
		
		X_LBR_NFDI di = new X_LBR_NFDI(getCtx(), nfLine.getLBR_NFDI_ID(), get_TrxName());
		if (di != null){
			String COD_DOC_IMP = "1";
	...
	public static RC170 createRC170(MLBRNotaFiscalLine nfLine, String COD_ITEM, String TIPO_ITEM, String UNID, int line)
	{
		...
		BigDecimal VL_DESC = MLBRNFLineTax.getTaxesDiscount(nfLine);
		String     IND_MOV = TIPO_ITEM.equals("09") ? "1" : "0"; //1=SEM MOV, 0=COM MOV

		/*
		 * ICMS
		 */
		X_LBR_NFLineTax taxLine = nfLine.getTax("ICMS");
		String CST_ICMS = (taxLine != null && taxLine.getLBR_TaxStatus() != null) ? taxLine.getLBR_TaxStatus().getName() : "00";
		BigDecimal VL_BC_ICMS    = taxLine != null ? taxLine.getlbr_TaxBaseAmt() : Env.ZERO;
		BigDecimal ALIQ_ICMS     = taxLine != null ? taxLine.getlbr_TaxRate() : Env.ZERO;
		BigDecimal VL_ICMS       = taxLine != null ? taxLine.getlbr_TaxAmt() : Env.ZERO;
		 ...
		 * ICMSST
		 */
		taxLine = nfLine.getTax("ICMSST");
		BigDecimal VL_BC_ICMS_ST = taxLine != null ? taxLine.getlbr_TaxBaseAmt() : Env.ZERO;;
		BigDecimal ALIQ_ST       = taxLine != null ? taxLine.getlbr_TaxRate() : Env.ZERO;;
		BigDecimal VL_ICMS_ST    = taxLine != null ? taxLine.getlbr_TaxAmt() : Env.ZERO;;
		

		/*
		 * IPI
		 */
		taxLine = nfLine.getTax("IPI");
		String CST_IPI = (taxLine != null && taxLine.getLBR_TaxStatus() != null) ? taxLine.getLBR_TaxStatus().getName() : "00";
		BigDecimal VL_BC_IPI = taxLine != null ? taxLine.getlbr_TaxBaseAmt() : Env.ZERO;
		BigDecimal ALIQ_IPI  = taxLine != null ? taxLine.getlbr_TaxRate() : Env.ZERO;;
		BigDecimal VL_IPI    = taxLine != null ? taxLine.getlbr_TaxAmt() : Env.ZERO;;

		
		/*
		 * PIS
		 */
		taxLine = nfLine.getTax("PIS");
		String CST_PIS 			= (taxLine != null && taxLine.getLBR_TaxStatus() != null) ? taxLine.getLBR_TaxStatus().getName() : "06";
		BigDecimal VL_BC_PIS 	= taxLine != null ? taxLine.getlbr_TaxBaseAmt() : Env.ZERO;
		BigDecimal ALIQ_PIS 	= taxLine != null ? taxLine.getlbr_TaxRate() : Env.ZERO;
		BigDecimal QUANT_BC_PIS = Env.ZERO; //TODO
		BigDecimal V_ALIQ_PIS 	= Env.ZERO; //TODO
		BigDecimal VL_PIS 		= taxLine != null ? taxLine.getlbr_TaxAmt() : Env.ZERO;


		/*
		 * COFINS
		 */
		taxLine = nfLine.getTax("COFINS");
		String CST_COFINS 		= (taxLine != null && taxLine.getLBR_TaxStatus() != null) ? taxLine.getLBR_TaxStatus().getName() : "06";
		BigDecimal VL_BC_COFINS = taxLine != null ? taxLine.getlbr_TaxBaseAmt() : Env.ZERO;
		BigDecimal ALIQ_COFINS 	= taxLine != null ? taxLine.getlbr_TaxRate() : Env.ZERO;
		BigDecimal QUANT_BC_COFINS 	= Env.ZERO; //TODO 
		BigDecimal V_ALIQ_COFINS	= Env.ZERO; //TODO
		BigDecimal VL_COFINS 	= taxLine != null ? taxLine.getlbr_TaxAmt() : Env.ZERO;

		/*
		 * Outras info
		 */
		String CFOP     = FRKUtils.tratarCFOP(nfLine.getLBR_CFOP_ID());
		String COD_NAT  = ""; 	//TODO		
		...
		BigDecimal VL_OPR = FRKUtils.getVL_OPER(nfLine);
	
		
		// NF Para Zona de Livre comÃ©rcio, zera os impostos
		if(FRKUtils.hasSuframa(nf.getlbr_BPSuframa()))
		{
			...
			ALIQ_PIS 	= Env.ZERO; 
			VL_PIS 		= Env.ZERO; 
			VL_BC_COFINS = Env.ZERO; 
			ALIQ_COFINS = Env.ZERO; 
			VL_COFINS	= Env.ZERO; 
	...
	public static RC172 createRC172(RC170 rc170, MLBRNotaFiscalLine nfLine){
		...
		BigDecimal VL_SERV_NT = FRKUtils.getVL_OPER(nfLine).subtract(nfLine.getICMSAmt());
	...
	public static RC510 createRC510(MLBRNotaFiscalLine nfLine, String COD_ITEM, String UNID, int line){
		...
		BigDecimal QTD = nfLine.getQty();
		BigDecimal VL_ITEM = nfLine.getLineTotalAmt();
		BigDecimal VL_DESC = FRKUtils.getDiscountAmtByLine(nfLine);
	...
	public static RD510 createRD510(MLBRNotaFiscalLine nfLine, String COD_ITEM, String UNID, int line){
		...
		BigDecimal QTD = nfLine.getQty();
		BigDecimal VL_ITEM = nfLine.getLineTotalAmt();
		BigDecimal VL_DESC = nfLine.getDiscount() == null ? Env.ZERO : nfLine.getDiscount();
	...
	{
		System.out.println("createRE111 - >>  INI");
		...
		for (X_LBR_ApuracaoICMSLine line :lines){
			...
			//
			X_LBR_ICMSBasis basis = new X_LBR_ICMSBasis(getCtx(), line.getLBR_ICMSBasis_ID(), get_TrxName());
			
			//
			if (basis.getType().equals("S"))
				isST = true;
		...
		System.out.println("createRE111 - >>  FIM");
	...
	public static RE530[] createRE530(Timestamp dateFrom)
	{
		...
		System.out.println("createRE530 - >>  INI");
	...
	public static RG110 createRG110(Timestamp dateFrom, Timestamp dateTo, 
		...
		BigDecimal IND_PER_SAI = VL_TRIB_EXP.divide(VL_TOTAL, FRKUtils.MCROUND);
	...
	/**
	 * Montar o sql para buscar os saldos de estoque do último 
	 * mês do ano que passou baseado na MStorage e MTransaction
	 * 
	 * @param AD_Org_ID
	 * @return sql para a finalidade descrita acima
	 */
	public static String getSQLInv(int AD_Org_ID)
	{
		...
		/*
		 * Retornar os Org_IDs das organizações que tem mesmo CNPJ, ex.: Barb e Metais, para agrupar o estoque
		 */
		sql += " AND AD_Org_ID IN (";
		FRKUtils.getOrgsPorCnpjSeparaPorVirgula(
				getCtx(), 
				MOrgInfo.get(getCtx(), AD_Org_ID, get_TrxName()).get_ValueAsString(I_W_AD_OrgInfo.COLUMNNAME_lbr_CNPJ), 
				get_TrxName());
		sql += ")";
