 ...
 *  	FR [ 3148235 ] - Improvements to Set Variable Workflow
	...
	private boolean performWork (Trx trx) throws Exception
	{
		...
		*/
		...
		else if (MWFNode.ACTION_SetVariable.equals(action))
		{
			...
			//	Record ID
			if(value.indexOf("@Record_ID@") != -1 && value.indexOf(" ") == -1)
			...
			else if(value.startsWith("@SQL="))
			{
				...
				else
				{
					...
					try
					{
						...
						if (rs.next())
							resultQuery = rs.getObject(1);
						else
							log.log(Level.WARNING, "No Result: " + sql);
					}
					catch (SQLException e)
					{
						log.log(Level.WARNING, "(" + sql + ") " + sql, e);
					}
					finally
					{
						DB.close(rs, stmt);
