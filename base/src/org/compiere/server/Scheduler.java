...
public class Scheduler extends AdempiereServer
{
	...
	private String runProcess(MProcess process) throws Exception
	{
		...
		/**
		 * 		Execute the process only if there is a classname or a procedure,
		 * 	ignore in case both fields are empty (report only).
		 */
		boolean isProcess = ((process.getProcedureName() != null && process.getProcedureName().length() > 0)
				|| (process.getClassname() != null && process.getClassname().length() > 0));
		...
		if ( isProcess && !process.processIt (pi, m_trx) ) // note, this call close the transaction, don't use m_trx below
		{
