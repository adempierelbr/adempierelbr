...
public class MTable extends X_AD_Table
{
	...
	private static final String[]	s_packages = new String[] {
		...
		"org.adempiere.model",
		"org.adempierelbr.model" //ADempiereLBR - APENAS PARA COMPATIBILIDADE COM SISTEMAS SEM SCRIPT EXECUTADO
	};
	...
	private static final String[]	s_special = new String[] {
		...
		"LBR_Tax", "org.adempierelbr.model.MTaxLBR"
	...
	public static Class<?> getClass (String tableName)
	{
		...
		if (!MEntityType.ENTITYTYPE_Dictionary.equals(entityType))
		{
			...
			{				
				...
				//Para tabelas do LBR e OSEB remover os padrões _
				if (entityType.equalsIgnoreCase("LBRA") || entityType.equalsIgnoreCase("OSEB")){
					clazz = getPOclass(etmodelpackage + ".M" + Util.replace(tableName.substring(tableName.indexOf('_')+1), "_", ""));
				}
				else{
		...
		{
			//ADempiereLBR - APENAS PARA COMPATIBILIDADE COM SISTEMAS SEM SCRIPT EXECUTADO
			if (entityType.equalsIgnoreCase("LBRA")){ //Model AdempiereLBR
		...
		//ADempiereLBR - APENAS PARA COMPATIBILIDADE COM SISTEMAS SEM SCRIPT EXECUTADO
		clazz = getPOclass("org.adempierelbr.model.X_" + tableName);
		if (clazz != null)
		{
			s_classCache.put(tableName, clazz);
			return clazz;
