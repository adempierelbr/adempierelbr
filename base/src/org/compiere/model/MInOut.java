...
public class MInOut extends X_M_InOut implements DocAction
{
	...
	public MInOut (MOrder order, int C_DocTypeShipment_ID, Timestamp movementDate)
	{
		...
		/**		KENOS - MM Returns		*/
		String baseType = DB.getSQLValueString(null,
				"SELECT COALESCE(DocBaseType, ' ') AS DocBaseType FROM C_DocType WHERE C_DocType_ID=?", 
				C_DocTypeShipment_ID);
		
		Boolean isDocTypeSOTrx = DB.getSQLValueString(null,
