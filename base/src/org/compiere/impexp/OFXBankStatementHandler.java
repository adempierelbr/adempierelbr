...
public abstract class OFXBankStatementHandler extends DefaultHandler
{
	 ...
	 */
	protected boolean attachInput(InputStream is, String enconding)
	{
		boolean isOfx1 = true;
		boolean result = false;
		
		if (enconding == null)
			enconding = Ini.getCharset().name();
		
		try
		{
			BufferedReader reader = new BufferedReader(new InputStreamReader(is, enconding));
			...
			if (isOfx1)
			{
				m_reader = new BufferedReader(new InputStreamReader(new OFX1ToXML(reader), enconding));
			}
			else
			{
				m_reader = reader;
