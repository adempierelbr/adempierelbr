...
public final class AppsAction extends AbstractAction
{
	 ...
	 *  @return Button
	 */
	public AbstractButton getButton (boolean small)
	{
		AbstractButton absButton = getButton();
		//
		if (small)
		{
			Icon smallIcon = getIcon(absButton.getName(), true);	//	small
			if (smallIcon == null)
				return absButton;
			//
			if (absButton instanceof CButton)
			{
				CButton button = (CButton) absButton;
				button.setIcon(smallIcon);
				absButton = button;
			}
			else if (absButton instanceof CToggleButton)
			{
				CToggleButton button = (CToggleButton) absButton;
				button.setIcon(smallIcon);
				absButton = button;
			}
		}
