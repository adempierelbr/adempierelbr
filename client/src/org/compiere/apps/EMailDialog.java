 ...
 *  				Reply-To configurado no MSysConfig com a tag INOVA_EMAIL_REPLYTO
 *
 */
	...
	implements ActionListener, VetoableChangeListener
{
	...
	public EMailDialog (Frame owner, String title, MUser from, String to, 
		String subject, String message, File attachment, String replyTo)
	{
		super (owner, title, true);
		commonInit(from, to, subject, message, attachment, replyTo);
	}	//	EmailDialog
	
	public EMailDialog (Frame owner, String title, MUser from, String to, 
			String subject, String message, File attachment)
	{
		this (owner, title, from, to, subject, message, attachment, null);
	}	//	EMailDialog
	...
	private void commonInit (MUser from, String to, 
			String subject, String message, File attachment)
	{
		commonInit ( from, to, subject, message, attachment, null);
	} 	//	commonInit
	...
	private ArrayList<File>	m_attachFile = new ArrayList<File>();
	/**	Logger			*/
	...
	private CButton bAttach = new CButton(Msg.getMsg(Env.getCtx(), "Attachment"));
	...
	void jbInit() throws Exception
	{
		...
		//
		bAttach.setIcon(Env.getImageIcon2("Attachment24"));
		bAttach.addActionListener(this);
		//
		confirmPanel.addComponent(bAttach);
		//
	...
	public void addAttachment (File attachment)
	{
		...
		else
		{
			String text = "";
			//
			if (fAttachment.getText() == null || fAttachment.getText().length() <= 0)
				text = attachment.getName();
			else
				text = fAttachment.getText() + ", " + attachment.getName();
			//
			lAttachment.setVisible(true);
			fAttachment.setVisible(true);
			fAttachment.setText(text);
			fAttachment.setReadWrite(false);
			//
			m_attachFile.add(attachment);
		}
	...
	{
		if (m_attachFile.size() > 0)
			return m_attachFile.get(0);
		else
			return null;
	}	//	getAttachment
	...
	public void actionPerformed(ActionEvent e)
	{
		if (e.getActionCommand().equals(ConfirmPanel.A_CANCEL))
			dispose();
		else if (e.getSource().equals(bAttach))
		{			
		...
		if (e.getActionCommand().equals(ConfirmPanel.A_OK))
		{
			...
			if (email != null)
			{
				...
				//	Attachment
				for (int pos=0; pos < m_attachFile.size(); pos++)
				{
					File file = m_attachFile.get(pos);
					//
