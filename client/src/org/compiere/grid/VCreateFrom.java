	...
	implements ActionListener, TableModelListener
{
	...
	public static VCreateFrom create (GridTab mTab)
	{
		...
		/**
		 * Manufacture - Industrialização BR
		 * */
		else if (AD_Table_ID == 259)
			retValue = new VCreateFromManufacture(mTab); // RMA
	...
	//LBR
	protected JLabel productionLabel = new JLabel();
	protected VLookup productionField;
...
//	public static final String SELECT_ALL_TOOLTIP = "Select all records";	
	...
	private void jbInit() throws Exception
	{
		...
		{
			parameterStdPanel.add(productionLabel, new GridBagConstraints(2, 0, 1, 1, 0.0, 0.0
					,GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL, new Insets(5, 0, 5, 5), 0, 0));
			parameterStdPanel.add(productionField, new GridBagConstraints(3, 0, 1, 1, 0.0, 0.0
				,GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL, new Insets(5, 0, 5, 5), 0, 0));
		}
...
//		selectAllButton.setToolTipText(Msg.getMsg(Env.getCtx(), SELECT_ALL_TOOLTIP));
	 ...
	 *  @throws Exception if Lookups cannot be initialized
	 */
	protected void initProduction (boolean forInvoice) throws Exception
	{
		//  load BPartner
		int AD_Column_ID = 3596;        //  C_Invoice.C_BPartner_ID
		MLookup lookup = MLookupFactory.get (Env.getCtx(), p_WindowNo, 0, AD_Column_ID, DisplayType.Search);
		productionField = new VLookup ("M_Production_ID", true, false, true, lookup);
		//
		//int C_BPartner_ID = Env.getContextAsInt(Env.getCtx(), p_WindowNo, "C_BPartner_ID");
		//productionField.setValue(new Integer(C_BPartner_ID));
	...
	protected void loadOrder (int C_Order_ID, boolean forInvoice)
	{
        ...
        if(!forInvoice && C_DocType_ID > 0)
        {
        	MDocType docType = new MDocType(Env.getCtx(), C_DocType_ID, null);
        	
        	Boolean isManu = (Boolean) docType.get_Value("lbr_IsManufactured");
        	Boolean isReturn = (Boolean) docType.get_Value("lbr_IsReturn");
        	
        	/**	No caso de retorno de industrialização o produto 
        	 * deve ser listado somente os produtos acabados
        	 * */
        	if (isManu != null && isReturn != null
        			&& isManu && isReturn)
        	{
        		sql = new StringBuffer("SELECT "
        				+ "pp.ProductionQty,"					//	1
        				+ "pp.ProductionQty,"	//	2
        				+ " p.C_UOM_ID,COALESCE(uom.UOMSymbol,uom.Name),"			//	3..4
        				+ " COALESCE(pp.M_Product_ID,0),p.Name,null,"	//	5..7
        				+ " null, 0 "								//	8..9
        				...
        				+ " WHERE M_ProductionPlan_ID IN (SELECT DISTINCT M_ProductionPlan_ID "
        				+ "   FROM M_ProductionLine WHERE M_ProductionLine_ID IN "
        				+ "   (SELECT M_ProductionLine_ID FROM C_OrderLine WHERE C_Order_ID=?))");
        	}
