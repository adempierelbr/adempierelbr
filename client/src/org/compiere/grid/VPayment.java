	...
	implements ActionListener
{
	...
	private boolean				m_Cash_As_Payment = false;
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		else if (e.getSource() == paymentCombo)
		{
			...
			if (pp != null)
			{
				...
				//	Na compra o Cartão de Crédito deve se comportar como Dinheiro
				if ((X_C_Order.PAYMENTRULE_CreditCard.toLowerCase().equals(s) || X_C_Order.PAYMENTRULE_Check.toLowerCase().equals(s))
						&& MSysConfig.getBooleanValue("LBR_CREDITCARDASCASHBOOK", false, Env.getAD_Client_ID(Env.getCtx())))
					s = X_C_Order.PAYMENTRULE_OnCredit.toLowerCase();
	...
	private boolean saveChangesInTrx(final String trxName){
		...
		String origPaymentRule = newPaymentRule;
		...
		//	Cartão de Crédito na entrada
		if ((X_C_Order.PAYMENTRULE_CreditCard.equals(newPaymentRule) || X_C_Order.PAYMENTRULE_Check.equals(newPaymentRule))
				&& MSysConfig.getBooleanValue("LBR_CREDITCARDASCASHBOOK", false, Env.getAD_Client_ID(Env.getCtx())))
			newPaymentRule = X_C_Order.PAYMENTRULE_OnCredit;
		...
		if (newPaymentRule.equals(X_C_Order.PAYMENTRULE_Cash) && !m_Cash_As_Payment)
		{
			...
			else
			{
				...
				if (m_cashLine == null)
				{
					...
					//	Completar CashBook
					if (vp.getValue().equals(X_C_Order.PAYMENTRULE_CreditCard))
					{
						String docStatus = cash.completeIt();
						cash.setDocStatus(docStatus);
						cash.save();
					}
				}
			}	//	have invoice
		...
		//
		if (!origPaymentRule.equals(m_PaymentRule))
			m_mTab.setValue("PaymentRule", origPaymentRule);
		//
	...
	private boolean checkMandatory()
	{
		...
		//	Cheque na entrada
		if ((X_C_Order.PAYMENTRULE_CreditCard.equals(PaymentRule) || X_C_Order.PAYMENTRULE_Check.equals(PaymentRule))
				&& MSysConfig.getBooleanValue("LBR_CREDITCARDASCASHBOOK", false, Env.getAD_Client_ID(Env.getCtx())))
			PaymentRule = X_C_Order.PAYMENTRULE_OnCredit;
