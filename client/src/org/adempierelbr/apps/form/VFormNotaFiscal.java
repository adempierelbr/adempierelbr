 ...
 */
public class VFormNotaFiscal extends CPanel
	implements FormPanel, ActionListener, VetoableChangeListener, ChangeListener, TableModelListener, ASyncProcess
{
	...
	/**	Logger			*/
	private static CLogger log = CLogger.getCLogger(VFormNotaFiscal.class);
	//
	...
	void jbInit() throws Exception
	{
		...
		selNorthPanel.add(lDateDoc, new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0
	...
	private void executeQuery()
	{
		...
		try
		{
			...
			while (rs.next())
			{
				...
				//  set values
				miniTable.setValueAt(new IDColumn(rs.getInt(1)), row, 0);   //  LBR_NotaFiscal_ID
				...
				miniTable.setValueAt(rs.getTimestamp(5), row, 4);           //  DateDoc
				//  prepare next
				row++;
			}
		}
	...
	public void vetoableChange(PropertyChangeEvent e)
	{
		...
		if (e.getPropertyName().equals("DateDoc"))
		{
			...
			fDateDoc.setValue(m_DateDoc);	//	display value
		...
		if (e.getPropertyName().equals("LBR_MatrixPrinter_ID"))
		{
			...
			fMatrixPrinter.setValue(m_LBR_MatrixPrinter_ID);	//	display value
		}
