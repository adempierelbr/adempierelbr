      ...
      /** if (LBR_DocPrint_ID == 0)
        {
			...
			setlbr_HasSubDoc (false);
// 'N'
			setlbr_IsSubDoc (false);
// 'N'
    ...
    {
      StringBuffer sb = new StringBuffer ("X_LBR_DocPrint[")
    ...
    {
		return (org.adempierelbr.model.I_LBR_MatrixPrinter)MTable.get(getCtx(), org.adempierelbr.model.I_LBR_MatrixPrinter.Table_Name)
			.getPO(getLBR_MatrixPrinter_ID(), get_TrxName());	}
	...
	public void setLBR_MatrixPrinter_ID (int LBR_MatrixPrinter_ID)
	{
		if (LBR_MatrixPrinter_ID < 1) 
			set_Value (COLUMNNAME_LBR_MatrixPrinter_ID, null);
		else 
			set_Value (COLUMNNAME_LBR_MatrixPrinter_ID, Integer.valueOf(LBR_MatrixPrinter_ID));
	}
	...
	public org.adempierelbr.model.I_LBR_DocPrint getlbr_SubDoc() throws RuntimeException
    {
		return (org.adempierelbr.model.I_LBR_DocPrint)MTable.get(getCtx(), org.adempierelbr.model.I_LBR_DocPrint.Table_Name)
			.getPO(getlbr_SubDoc_ID(), get_TrxName());	}
	...
	{
		if (lbr_SubDoc_ID < 1) 
			set_Value (COLUMNNAME_lbr_SubDoc_ID, null);
		else 
			set_Value (COLUMNNAME_lbr_SubDoc_ID, Integer.valueOf(lbr_SubDoc_ID));
	}
    ...
    {
		return (org.adempierelbr.model.I_LBR_DocPrint)MTable.get(getCtx(), org.adempierelbr.model.I_LBR_DocPrint.Table_Name)
			.getPO(getlbr_SubDoc2_ID(), get_TrxName());	}
	...
	{
		if (lbr_SubDoc2_ID < 1) 
			set_Value (COLUMNNAME_lbr_SubDoc2_ID, null);
		else 
			set_Value (COLUMNNAME_lbr_SubDoc2_ID, Integer.valueOf(lbr_SubDoc2_ID));
	}
