 ...
 */
public class ModelWrapperGenerator
{
	...
	public ModelWrapperGenerator(int AD_Table_ID, String directory, String packageName) {
		this(AD_Table_ID,directory,packageName,null, null);
	}

	public ModelWrapperGenerator(int AD_Table_ID, String directory, String packageName, String entityType, String preffix) {
		...
		// Header
		String tableName = createHeader(AD_Table_ID, sb, mandatory, preffix);
	 ...
	 * @return class name
	 */
	private String createHeader(int AD_Table_ID, StringBuffer sb, StringBuffer mandatory, String preffix)
	{	
		if (preffix == null || preffix.equals(""))
			preffix = "";
		...
		start.append("/** Generated Interface for ").append(tableName).append("\n")
			 ...
			 .append(" */\n")
			 .append("public interface ").append(className).append(" extends I_").append(tableName).append(" {").append("\n");
	...
	{
		String fieldName;
		if (columnName.endsWith("_CR") || columnName.endsWith("_DR")){
			fieldName = columnName.substring(columnName.length() - 2) + "_" +
			            columnName.substring(0, columnName.length() - 3);
		}

		else if (columnName.endsWith("_ID_To"))
