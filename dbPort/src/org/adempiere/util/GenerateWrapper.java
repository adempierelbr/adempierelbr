...
public class GenerateWrapper
{
	...
	/**	Logger			*/
	private static CLogger	log	= CLogger.getCLogger (GenerateWrapper.class);
	...
	public static void main (String[] args)
	{
		...
		tableLike = "'C_BPartner'";	//	All tables
		...
		sql.insert(0, "SELECT AD_Table_ID, EntityType "
			...
			+ " AND EXISTS (SELECT '1' FROM AD_Column c WHERE c.AD_Table_ID=AD_Table.AD_Table_ID AND c.EntityType IN ("+entityTypeColumn+")) AND ");
		...
		try
		{
			...
			while (rs.next())
			{
				...
				new ModelWrapperGenerator(rs.getInt(1), classDirectory, packageName, entityTypeColumn, preffix);
				count++;
			}
 		}
		catch (Exception e)
