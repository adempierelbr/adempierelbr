...
//	private Timestamp 		s_run = new Timestamp(System.currentTimeMillis());
	...
	private String createHeader (int AD_Table_ID, StringBuffer sb, StringBuffer mandatory, String packageName)
	{
		...
		start.append("import java.util.*;"
			...
			+ " *  @version ").append(Compiere.getVersion()).append(" - $Id$")
...
//			+ "}"	//	Constructor End
			...
			+ "protected static KeyNamePair Model = new KeyNamePair(").append("Table_ID").append(",\"").append(tableName).append("\");\n"
			//	
	...
	//	CLogMgt.setLevel(Level.ALL);
		...
		//	first parameter
		// Kenos
		String directory = "dbPort/src/org/compiere/model/";
		//String directory = "/Compiere2/compiere-all/dbPort/src/org/compiere/model/";
    if (args.length > 0)
			directory = args[0];
		...
		//	second parameter
		// Kenos
		//String packageName = "compiere.model"; LINHA ORIGINAL DO COMPIERE
